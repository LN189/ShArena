...
def file_view(request,username,projectname,filename):
	...
	return render(request,'user_login/openfile.html',{'matter' : matter ,'path':path ,'filename':filename ,'user' : username , 'pro' : projectname})
	
def update_db(request,username,projectname,filename,matter):
	path = 'media/user_' + str(username) + '/' + str(projectname) + '/' + str(filename)
	raw = open(path, "r+")
